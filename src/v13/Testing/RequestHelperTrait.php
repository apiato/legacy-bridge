...
use App\Ship\LegacyBridge\Testing\Exceptions\MissingTestEndpointException;
use App\Ship\LegacyBridge\Testing\Exceptions\UndefinedMethodException;
use App\Ship\LegacyBridge\Testing\Exceptions\WrongEndpointFormatException;
...
trait RequestHelperTrait
{
    ...
    public function makeCall(array $data = [], array $headers = []): TestResponse
    {
        ...
        switch ($verb) {
            ...
            case 'delete':
                break;
            default:
                throw UndefinedMethodException::create(null, 'Unsupported HTTP Verb (' . $verb . ')!');
        }
    ...
    {
        if (!$this->getEndpoint()) {
            throw MissingTestEndpointException::create(null, null);
        }
    }
    ...
    private function validateEndpointFormat(string $separator): void
    {
        if (!strpos($this->getEndpoint(), $separator)) {
            throw WrongEndpointFormatException::create(null, null);
        }
    }
